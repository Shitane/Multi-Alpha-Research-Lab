//+------------------------------------------------------------------+
//| A13 Dual MA Module v1.00                                        |
//| Signal/state module extracted from verified A13 NoOrders core.  |
//| SAFETY: no broker execution functions.                           |
//+------------------------------------------------------------------+
#ifndef __A13_DUAL_MA_MODULE_V1_00_MQH__
#define __A13_DUAL_MA_MODULE_V1_00_MQH__

struct SA13Brick { double open,close; int direction,run; };
const int A13_MAX_BRICKS_PER_TICK=4096;
...
class CA13DualMA
{
 ...
 public:
  CA13DualMA(){m_tick_size=0;m_effective_brick=0;m_size=0;m_close=0;m_dir=0;m_run=0;m_fast=0;m_slow=0;m_count=0;m_entry_run=0;m_min_sep=0;m_anchor=false;m_failed=false;}
  bool Init(const double tick_size,const double requested_brick,const int fast_period,const int slow_period,const double min_sep_bricks,const int entry_run)
  {
   ...
   if(fast_period<2||fast_period>100||slow_period<3||slow_period>250||fast_period>=slow_period||entry_run<1||entry_run>50||min_sep_bricks<0)return false;
  ...
  int PushBid(const double price,int &brick_count,int &final_alignment)
  {
   ...
   if(count>A13_MAX_BRICKS_PER_TICK){m_failed=true;return 0;}if(count==0)return 0;
   ...
   for(int i=0;i<brick_count;i++)
   {
    ...
    SA13Brick b;b.open=op*m_tick_size;b.close=m_close*m_tick_size;b.direction=dir;b.run=m_run;
