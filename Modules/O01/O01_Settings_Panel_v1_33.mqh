//+------------------------------------------------------------------+
//| O01_Settings_Panel_v1_33.mqh                                   |
//| Semi-transparent dark runtime panel, white-first typography.     |
//| APPLY/SAVE/LOAD/REFRESH. NO ORDERS.                              |
//+------------------------------------------------------------------+
#ifndef O01_SETTINGS_PANEL_V1_33_MQH
#define O01_SETTINGS_PANEL_V1_33_MQH
#include "O01_Runtime_Settings_v1_10.mqh"
#include <Canvas\Canvas.mqh>

class CO01SettingsPanel133{
 string p; CO01SettingsStore110 st; CCanvas cv; bool cvReady;
 color C_TEXT,C_MUTED,C_HEAD,C_ACCENT,C_EDIT_BG,C_BORDER,C_BUTTON;
 void CanvasCreate(){
  string n=p+"BG_CANVAS";if(cvReady){cv.Destroy();cvReady=false;}
  cvReady=cv.CreateBitmapLabel(0,0,n,6,18,610,480,COLOR_FORMAT_ARGB_NORMALIZE);
  if(!cvReady){Print("O01 panel canvas creation failed");return;}
  ...
  uchar a=145;cv.Erase(ColorToARGB(C'18,22,28',a));
  uint fr=ColorToARGB(C'105,120,135',200),sp=ColorToARGB(C'70,90,108',165);
  cv.Rectangle(0,0,609,479,fr);
  cv.Line(12,47,598,47,sp);cv.Line(12,151,598,151,sp);cv.Line(12,258,598,258,sp);cv.Line(12,347,598,347,sp);cv.Line(304,347,304,427,sp);
  cv.Update(false);
 }
...
public:
 void Create(const SO01RuntimeSettings110&s){
  p="O01CFG133_";cvReady=false;st.SetFile("O01_GSG_RSI30_Settings_v1_10.csv");
  ...
  CanvasCreate();
  L("TITLE",18,27,"O01 GOLD SESSION GUARD - RUNTIME PANEL v1.33",C_ACCENT,10);
  L("SAFE",405,28,"NO ORDERS / VIRTUAL NOT FILL",C_HEAD,8);
  L("SUB",18,47,"Settings / Research Panel",C_MUTED,8);
  H("ENTRY",24,73,"ENTRY / FILTER");
  ...
  H("SAFETY",24,377,"SAFETY / DD");
  F("WARN",24,397,"Warning DD %",(string)s.warning_dd,100,48);F("PAUSE",166,397,"Grid Pause %",(string)s.pause_grid_dd,92,42);
  F("CLOSE",24,420,"Emergency %",(string)s.emergency_close_dd,102,52);
  H("TIME",322,377,"TIME / NEWS");
