//+------------------------------------------------------------------+
//| O01_Settings_Panel_v1_41.mqh                                    |
//| STEP 3: fixed-layout panel body.                                 |
//| Trading settings/behavior remain compatible with v1.33.          |
//+------------------------------------------------------------------+
#ifndef O01_SETTINGS_PANEL_V1_41_MQH
#define O01_SETTINGS_PANEL_V1_41_MQH
...
public:
 ...
 int Event(int id,string name,SO01RuntimeSettings110&s){if(id!=CHARTEVENT_OBJECT_CLICK)return 0;if(name==p+"APPLY")return Pull(s)?1:-1;if(name==p+"SAVE"){if(!Pull(s))return-1;return st.Save(s)?2:-2;}if(name==p+"LOAD"){if(!st.Load(s))return-3;Delete();Create(s,identity,theme);return 3;}if(name==p+"RESET"){Delete();Create(s,identity,theme);return 4;}return 0;}
 void RefreshIdentity(const SMA140StrategyIdentity &strategy_cfg,const SMA140PanelTheme &appearance){identity=strategy_cfg;theme=appearance;Delete();Create(*(SO01RuntimeSettings110*)NULL,identity,theme);}
};
#endif
