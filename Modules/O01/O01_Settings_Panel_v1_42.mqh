...
class CO01SettingsPanel141{
 ...
 void E(string id,int x,int y,string v,int w=64){
  ...
  ObjectSetInteger(0,n,OBJPROP_BGCOLOR,C_EDIT_BG);ObjectSetInteger(0,n,OBJPROP_COLOR,theme.text);ObjectSetInteger(0,n,OBJPROP_BORDER_COLOR,theme.border);ObjectSetInteger(0,n,OBJPROP_FONTSIZE,theme.font_size);ObjectSetInteger(0,n,OBJPROP_ALIGN,ALIGN_CENTER);ObjectSetInteger(0,n,OBJPROP_READONLY,false);ObjectSetInteger(0,n,OBJPROP_SELECTABLE,true);ObjectSetInteger(0,n,OBJPROP_SELECTED,false);ObjectSetInteger(0,n,OBJPROP_HIDDEN,false);ObjectSetInteger(0,n,OBJPROP_ZORDER,20);ObjectSetString(0,n,OBJPROP_TEXT,v);
 }
...
public:
 ...
 int Event(int id,string name,SO01RuntimeSettings110&s){if(id==CHARTEVENT_OBJECT_CLICK&&StringFind(name,p)==0&&ObjectGetInteger(0,name,OBJPROP_TYPE)==OBJ_EDIT){ObjectSetInteger(0,name,OBJPROP_SELECTED,true);ChartRedraw();return 10;}if(id!=CHARTEVENT_OBJECT_CLICK)return 0;if(name==p+"APPLY")return Pull(s)?1:-1;if(name==p+"SAVE"){if(!Pull(s))return-1;return st.Save(s)?2:-2;}if(name==p+"LOAD"){if(!st.Load(s))return-3;Delete();Create(s,identity,theme);return 3;}if(name==p+"RESET"){Delete();Create(s,identity,theme);return 4;}return 0;}
