...
public:
 ...
 int Event(int id,string name,SO01RuntimeSettings110&s){if(id==CHARTEVENT_OBJECT_CLICK&&StringFind(name,p)==0&&ObjectGetInteger(0,name,OBJPROP_TYPE)==OBJ_EDIT){ObjectSetInteger(0,name,OBJPROP_READONLY,false);ObjectSetInteger(0,name,OBJPROP_SELECTED,true);ChartRedraw();return 0;}if(id==CHARTEVENT_OBJECT_ENDEDIT&&StringFind(name,p)==0&&ObjectGetInteger(0,name,OBJPROP_TYPE)==OBJ_EDIT){ObjectSetInteger(0,name,OBJPROP_SELECTED,false);ChartRedraw();return 0;}if(id!=CHARTEVENT_OBJECT_CLICK)return 0;if(name==p+"APPLY")return Pull(s)?1:-1;if(name==p+"SAVE"){if(!Pull(s))return-1;return st.Save(s)?2:-2;}if(name==p+"LOAD"){if(!st.Load(s))return-3;Delete();Create(s,identity,theme);return 3;}if(name==p+"RESET"){Delete();Create(s,identity,theme);return 4;}return 0;}
