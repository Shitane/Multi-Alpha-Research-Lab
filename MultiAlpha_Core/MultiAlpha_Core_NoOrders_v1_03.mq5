...
//| Unified A10/A11/A12/A15/A16 host using MultiAlpha interface.     |
//| Research only. NO ORDERS. Select Alpha with InpAlpha.            |
//+------------------------------------------------------------------+
#property strict

#include "..\\..\\Include\\MultiAlpha_Interface_v1_00.mqh"
#include "..\\..\\Include\\A10_Bollinger_Module_v1_00.mqh"
...
CA10ModeEngine a10_breakout;
CA10ModeEngine a10_reentry;
CA10ModeEngine a10_midline;
CA10ModeEngine a10_squeeze;
...
string A10_ModeName(const int mode)
  {
   if(mode==A10_BREAKOUT) return "Breakout";
   if(mode==A10_REENTRY) return "Re-entry";
   if(mode==A10_MIDLINE) return "Midline";
   if(mode==A10_SQUEEZE) return "Squeeze";
   return "Unknown";
  }
...
bool A10_ModeEnabled(const int mode)
  {
   if(mode==A10_BREAKOUT) return a10_breakout.Enabled();
   if(mode==A10_REENTRY) return a10_reentry.Enabled();
   if(mode==A10_MIDLINE) return a10_midline.Enabled();
   if(mode==A10_SQUEEZE) return a10_squeeze.Enabled();
   return false;
  }
...
double A10_ModeBrick(const int mode)
  {
   if(mode==A10_BREAKOUT) return a10_breakout.Brick();
   if(mode==A10_REENTRY) return a10_reentry.Brick();
   if(mode==A10_MIDLINE) return a10_midline.Brick();
   if(mode==A10_SQUEEZE) return a10_squeeze.Brick();
   return 0.0;
  }

//+------------------------------------------------------------------+
double A10_ModeTP(const int mode)
  {
   if(mode==A10_BREAKOUT) return a10_breakout.TP();
   if(mode==A10_REENTRY) return a10_reentry.TP();
   if(mode==A10_MIDLINE) return a10_midline.TP();
   if(mode==A10_SQUEEZE) return a10_squeeze.TP();
   return 0.0;
  }

//+------------------------------------------------------------------+
double A10_ModeSL(const int mode)
  {
   if(mode==A10_BREAKOUT) return a10_breakout.SL();
   if(mode==A10_REENTRY) return a10_reentry.SL();
   if(mode==A10_MIDLINE) return a10_midline.SL();
   if(mode==A10_SQUEEZE) return a10_squeeze.SL();
   return 0.0;
  }

//+------------------------------------------------------------------+
int A10_ModeMaxHold(const int mode)
  {
   if(mode==A10_BREAKOUT) return a10_breakout.Hold();
   if(mode==A10_REENTRY) return a10_reentry.Hold();
   if(mode==A10_MIDLINE) return a10_midline.Hold();
   if(mode==A10_SQUEEZE) return a10_squeeze.Hold();
   return 0;
  }
...
int A10_ModeCooldownLeft(const int mode)
  {
   if(mode==A10_BREAKOUT) return a10_breakout.Cooldown();
   if(mode==A10_REENTRY) return a10_reentry.Cooldown();
   if(mode==A10_MIDLINE) return a10_midline.Cooldown();
   if(mode==A10_SQUEEZE) return a10_squeeze.Cooldown();
   return 0;
  }
...
void A10_StartModeCooldown(const int mode)
  {
   if(mode==A10_BREAKOUT) a10_breakout.StartCooldown();
   else if(mode==A10_REENTRY) a10_reentry.StartCooldown();
   else if(mode==A10_MIDLINE) a10_midline.StartCooldown();
   else if(mode==A10_SQUEEZE) a10_squeeze.StartCooldown();
  }
...
bool A10_ModeSpreadIsAcceptable(const int mode,const MqlTick &tick)
  {
   ...
   if(mode==A10_BREAKOUT) max_fraction=a10_breakout.Spread();
   else if(mode==A10_REENTRY) max_fraction=a10_reentry.Spread();
   else if(mode==A10_MIDLINE) max_fraction=a10_midline.Spread();
   else if(mode==A10_SQUEEZE) max_fraction=a10_squeeze.Spread();
   else return false;
   ...
   for(int owner=0;owner<4;owner++)
     {
      ...
      const int direction=a10_v_dir[owner];
      const double executable=(direction>0 ? tick.bid : tick.ask);
      const double brick=A10_ModeBrick(owner);
      const double move=direction*(executable-a10_v_price[owner]);
      if(A10_ModeTP(owner)*brick>0.0 && move>=A10_ModeTP(owner)*brick) { A10_RequestExit("TP",owner,a10_v_ticket[owner]); continue; }
      if(A10_ModeSL(owner)*brick>0.0 && move<=-A10_ModeSL(owner)*brick) { A10_RequestExit("SL",owner,a10_v_ticket[owner]); continue; }
      const int max_hold=A10_ModeMaxHold(owner);
      if(max_hold>0 && a10_v_time[owner]>0 && (long)(TimeCurrent()-a10_v_time[owner])>=(long)max_hold*60)
         A10_RequestExit("TIME",owner,a10_v_ticket[owner]);
     }
  }

//+------------------------------------------------------------------+
void A10_ProcessOwnerSignalExit(const int owner,const SA10TickResult &result)
  {
   if(result.completed<=0 || owner<0 || owner>3 || a10_exit_pending[owner]) return;

   ulong ticket=0,magic=0;
   long type=-1;
   double volume=0.0,open_price=0.0;
   datetime open_time=0;
   if(!A10_FindModePosition(owner,ticket,type,volume,open_price,open_time,magic)) return;

   const int position_direction=(type==POSITION_TYPE_BUY ? 1 : -1);
   if(owner==A10_REENTRY && result.bb_ready)
     {
      if(position_direction>0 && result.final_close>=result.bb_mid) A10_RequestExit("BB_MID",owner,ticket);
      else if(position_direction<0 && result.final_close<=result.bb_mid) A10_RequestExit("BB_MID",owner,ticket);
     }
   else if(result.raw_signal!=0 && result.raw_signal==-position_direction)
     {
      A10_RequestExit("BB_OPPOSITE",owner,ticket);
     }
  }
...
void A10_UpdateAllModesAndTrade(const double price,const MqlTick &tick)
  {
   ...
   A10_ProcessOwnerSignalExit(A10_BREAKOUT,r0);
   A10_ProcessOwnerSignalExit(A10_REENTRY,r1);
   A10_ProcessOwnerSignalExit(A10_MIDLINE,r2);
   A10_ProcessOwnerSignalExit(A10_SQUEEZE,r3);
...
int InitA10()
  {
   ...
   if(!a10_breakout.Configure(InpA10BreakoutEnabled,A10_BREAKOUT,tick_size,
                            InpA10BreakoutBrickSize,InpA10BreakoutBBPeriod,InpA10BreakoutDeviation,1.0,
                            InpA10BreakoutEntryRun,InpA10BreakoutTP,InpA10BreakoutSL,
                            InpA10BreakoutMaxHold,InpA10BreakoutCooldown,InpA10BreakoutMaxSpread) ||
      !a10_reentry.Configure(InpA10ReentryEnabled,A10_REENTRY,tick_size,
                           InpA10ReentryBrickSize,InpA10ReentryBBPeriod,InpA10ReentryDeviation,1.0,
                           InpA10ReentryEntryRun,InpA10ReentryTP,InpA10ReentrySL,
                           InpA10ReentryMaxHold,InpA10ReentryCooldown,InpA10ReentryMaxSpread) ||
      !a10_midline.Configure(InpA10MidlineEnabled,A10_MIDLINE,tick_size,
                           InpA10MidlineBrickSize,InpA10MidlineBBPeriod,InpA10MidlineDeviation,1.0,
                           InpA10MidlineEntryRun,InpA10MidlineTP,InpA10MidlineSL,
                           InpA10MidlineMaxHold,InpA10MidlineCooldown,InpA10MidlineMaxSpread) ||
      !a10_squeeze.Configure(InpA10SqueezeEnabled,A10_SQUEEZE,tick_size,
                           InpA10SqueezeBrickSize,InpA10SqueezeBBPeriod,InpA10SqueezeDeviation,InpA10SqueezeMaxWidth,
                           InpA10SqueezeEntryRun,InpA10SqueezeTP,InpA10SqueezeSL,
                           InpA10SqueezeMaxHold,InpA10SqueezeCooldown,InpA10SqueezeMaxSpread))
     {
...
int OnInit()
  {
   if(InpAlpha!=ALPHA_A10 && InpAlpha!=ALPHA_A11 && InpAlpha!=ALPHA_A12 && InpAlpha!=ALPHA_A15 && InpAlpha!=ALPHA_A16_TEMPLATE) return INIT_PARAMETERS_INCORRECT;
