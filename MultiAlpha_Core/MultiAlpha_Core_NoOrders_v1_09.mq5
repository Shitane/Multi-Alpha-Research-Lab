//+------------------------------------------------------------------+
//| MultiAlpha_Core_NoOrders_v1_09.mq5                              |
//| Unified A10/A11/A12/A13/A14/A15/A16 host using MultiAlpha interface.     |
...
#include "..\\..\\Include\\A14\\A14_Entry_Module_v1_00.mqh"
#include "..\\..\\Include\\A14\\A14_Exit_Module_v1_00.mqh"
...
// A14 parameters (verified original defaults)
input group "A14 Renko Momentum"
input double InpA14BrickSize=6.0;
input int InpA14MomentumPeriod=8;
input double InpA14MomentumThreshold=5.0;
input double InpA14TakeProfitBricks=10.0;
input double InpA14StopLossBricks=2.0;
input int InpA14MaxHoldMinutes=1440;
input int InpA14CooldownBricks=3;
input double InpA14MaxSpreadFraction=0.35;
...
// A14 state
CA14EntryModule g_a14_entry;
CA14ExitModule  g_a14_exit;
...
int InitA12()
  {
   ...
   if(!g_a12_exit.Configure(g_a12_effective_brick,InpA12TakeProfitBricks,
                            InpA12StopLossBricks,InpA12MaxHoldMinutes))
      return INIT_PARAMETERS_INCORRECT;
   return INIT_SUCCEEDED;
  }
...
int InitA13()
  {
   ...
   if(!g_a13_exit.Configure(g_a13.EffectiveBrick(),InpA13TakeProfitBricks,
                            InpA13StopLossBricks,InpA13MaxHoldMinutes))
      return INIT_PARAMETERS_INCORRECT;
   return INIT_SUCCEEDED;
  }
...
int InitA14()
  {
   if(!MathIsValidNumber(InpA14BrickSize) || InpA14BrickSize<_Point ||
      InpA14MomentumPeriod<2 || InpA14MomentumPeriod>200 ||
      !MathIsValidNumber(InpA14MomentumThreshold) || InpA14MomentumThreshold<=0.0 ||
      !MathIsValidNumber(InpA14TakeProfitBricks) || InpA14TakeProfitBricks<=0.0 ||
      !MathIsValidNumber(InpA14StopLossBricks) || InpA14StopLossBricks<=0.0 ||
      InpA14MaxHoldMinutes<0 || InpA14CooldownBricks<0 ||
      !MathIsValidNumber(InpA14MaxSpreadFraction) || InpA14MaxSpreadFraction<=0.0)
      return INIT_PARAMETERS_INCORRECT;
   const double tick_size=SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_SIZE);
   if(!MathIsValidNumber(tick_size) || tick_size<=0.0) return INIT_FAILED;
   const double units_raw=InpA14BrickSize/tick_size;
   if(!MathIsValidNumber(units_raw) || units_raw<0.5 || units_raw>1.0e9) return INIT_FAILED;
   const long units=(long)MathMax(1.0,MathRound(units_raw));
   if(!g_a14_entry.Init(tick_size,units,InpA14MomentumPeriod,InpA14MomentumThreshold,InpA14MaxSpreadFraction))
      return INIT_FAILED;
   g_a14_exit.Init(InpA14TakeProfitBricks,InpA14StopLossBricks,InpA14MaxHoldMinutes,g_a14_entry.BrickSize());
   return INIT_SUCCEEDED;
  }
...
int OnInit()
  {
   if(InpAlpha!=ALPHA_A10 && InpAlpha!=ALPHA_A11 && InpAlpha!=ALPHA_A12 && InpAlpha!=ALPHA_A13 && InpAlpha!=ALPHA_A14 && InpAlpha!=ALPHA_A15 && InpAlpha!=ALPHA_A16_TEMPLATE) return INIT_PARAMETERS_INCORRECT;
   ...
   else if(InpAlpha==ALPHA_A14) rc=InitA14();
...
void TickA11(const SMultiAlphaMarket &m)
  {
   ...
   if(signal==-1)
     {
      const ENUM_POSITION_TYPE current_type=(g_position.direction>0 ? POSITION_TYPE_BUY : POSITION_TYPE_SELL);
      const ENUM_A11_EXIT_REASON exit_reason=g_a11_exit.OnNoSignal(
         InpA11UseFastMAExit,g_position.is_open,current_type,g_a11.ClosedPrice(),g_a11.FastClosed());
      if(exit_reason!=A11_EXIT_NONE)
        {
         ...
         PrintFormat("[MULTI_ALPHA_CORE_EXIT] alpha=11 no=%I64d time=%s time_msc=%I64d dir=%d price=%.8f reason=%s NO_ORDERS=1 VIRTUAL_NOT_FILL=1",
            ...
            g_a11_exit.ReasonText(exit_reason));
         ClearPosition();
        }
      return;
     }
   ...
   const ENUM_POSITION_TYPE current_type=(g_position.direction>0 ? POSITION_TYPE_BUY : POSITION_TYPE_SELL);
   const ENUM_A11_EXIT_REASON exit_reason=g_a11_exit.OnSignal(g_position.is_open,current_type,signal);
...
void TickA12(const SMultiAlphaMarket &m)
  {
   ...
   if(g_position.is_open)
     {
      const ENUM_A12_EXIT_REASON r=g_a12_exit.TickDecision(
         g_position.is_open,g_position.direction,g_position.entry_price,g_position.entry_time,
         m.bid,m.ask,m.time);
      if(r!=A12_EXIT_NONE)
        {
         EmitExit(m,g_a12_exit.ReasonText(r),d);
         ApplyDecision(d);
         closed=true;
        }
     }
   ...
   if(g_position.is_open)
     {
      const ENUM_A12_EXIT_REASON r=g_a12_exit.CompletedRenkoDecision(
         g_position.is_open,g_position.direction,final_raw);
      if(r!=A12_EXIT_NONE)
        {
         EmitExit(m,g_a12_exit.ReasonText(r),d);
         ApplyDecision(d);
...
void TickA13(const SMultiAlphaMarket &m)
  {
   ...
   // Price/time exits remain active even if the Renko builder has failed.
   if(g_position.is_open)
     {
      MqlTick tick={};
      tick.bid=m.bid; tick.ask=m.ask; tick.time=m.time; tick.time_msc=m.time_msc;
      const EA13ExitDecision r=g_a13_exit.TickDecision(
         g_position.direction,g_position.entry_price,g_position.entry_time,tick);
      if(r!=A13_EXIT_NONE)
        {
         EmitExit(m,g_a13_exit.ReasonText(r),d); ApplyDecision(d);
         g_cooldown=InpA13CooldownBricks; closed=true;
        }
     }

   if(g_a13.Failed()) return;
   ...
   if(g_a13.Failed())
     {
      g_failed=true;
      Print("[MULTI_ALPHA_CORE_ERROR] alpha=13 renko_builder_stopped_new_entries_paused NO_ORDERS=1");
      return;
     }
   if(n==0) return;
   g_bricks+=n;
   ...
   if(g_position.is_open)
     {
      const EA13ExitDecision r=g_a13_exit.CompletedRenkoDecision(g_position.direction,alignment);
      if(r!=A13_EXIT_NONE)
        {
         EmitExit(m,g_a13_exit.ReasonText(r),d); ApplyDecision(d);
...
void TickA14(const SMultiAlphaMarket &m,const MqlTick &tick)
  {
   bool closed=false;
   SMultiAlphaDecision d; MultiAlphaDecisionClear(d,ALPHA_A14);

   const string exit_reason=g_a14_exit.Check(tick,g_position.is_open,g_position.direction,
                                             g_position.entry_price,g_position.entry_time);
   if(exit_reason!="")
     {
      EmitExit(m,exit_reason,d);
      ApplyDecision(d);
      g_cooldown=InpA14CooldownBricks;
      closed=true;
     }

   const int signal=g_a14_entry.Process(tick,g_position.is_open,closed,g_cooldown);
   g_bricks=g_a14_entry.Bricks();
   g_raw=g_a14_entry.Raw();
   g_spread_blocks=g_a14_entry.SpreadBlocks();
   g_blocks=g_a14_entry.CooldownBlocks()+g_spread_blocks;
   if(g_a14_entry.Failed())
     {
      g_failed=true;
      return;
     }
   ...
   MultiAlphaDecisionClear(d,ALPHA_A14);
   d.action=ALPHA_ACTION_ENTRY;
   d.direction=signal;
   d.reason="A14 RENKO MOMENTUM";
   d.decision_price=(signal>0 ? m.ask : m.bid);
   d.time=m.time; d.time_msc=m.time_msc;
   ApplyDecision(d);
  }
...
void OnTick()
  {
   if(g_failed && InpAlpha!=ALPHA_A13 && InpAlpha!=ALPHA_A14) return;
   ...
   else if(InpAlpha==ALPHA_A14) TickA14(m,tick);
