...
CA10ModeEngine g_breakout;
CA10ModeEngine g_reentry;
CA10ModeEngine g_midline;
CA10ModeEngine g_squeeze;
...
bool ModeEnabled(const int mode)
  {
   if(mode==A10_BREAKOUT) return g_breakout.Enabled();
   if(mode==A10_REENTRY) return g_reentry.Enabled();
   if(mode==A10_MIDLINE) return g_midline.Enabled();
   if(mode==A10_SQUEEZE) return g_squeeze.Enabled();
   return false;
  }
...
double ModeBrick(const int mode)
  {
   if(mode==A10_BREAKOUT) return g_breakout.Brick();
   if(mode==A10_REENTRY) return g_reentry.Brick();
   if(mode==A10_MIDLINE) return g_midline.Brick();
   if(mode==A10_SQUEEZE) return g_squeeze.Brick();
   return 0.0;
  }
...
double ModeTP(const int mode)
  {
   if(mode==A10_BREAKOUT) return g_breakout.TP();
   if(mode==A10_REENTRY) return g_reentry.TP();
   if(mode==A10_MIDLINE) return g_midline.TP();
   if(mode==A10_SQUEEZE) return g_squeeze.TP();
   return 0.0;
  }
...
double ModeSL(const int mode)
  {
   if(mode==A10_BREAKOUT) return g_breakout.SL();
   if(mode==A10_REENTRY) return g_reentry.SL();
   if(mode==A10_MIDLINE) return g_midline.SL();
   if(mode==A10_SQUEEZE) return g_squeeze.SL();
   return 0.0;
  }
...
int ModeMaxHold(const int mode)
  {
   if(mode==A10_BREAKOUT) return g_breakout.Hold();
   if(mode==A10_REENTRY) return g_reentry.Hold();
   if(mode==A10_MIDLINE) return g_midline.Hold();
   if(mode==A10_SQUEEZE) return g_squeeze.Hold();
   return 0;
  }
...
int ModeCooldownLeft(const int mode)
  {
   if(mode==A10_BREAKOUT) return g_breakout.Cooldown();
   if(mode==A10_REENTRY) return g_reentry.Cooldown();
   if(mode==A10_MIDLINE) return g_midline.Cooldown();
   if(mode==A10_SQUEEZE) return g_squeeze.Cooldown();
   return 0;
  }
...
void StartModeCooldown(const int mode)
  {
   if(mode==A10_BREAKOUT) g_breakout.StartCooldown();
   else if(mode==A10_REENTRY) g_reentry.StartCooldown();
   else if(mode==A10_MIDLINE) g_midline.StartCooldown();
   else if(mode==A10_SQUEEZE) g_squeeze.StartCooldown();
  }
...
bool ModeSpreadIsAcceptable(const int mode,const MqlTick &tick)
  {
   ...
   if(mode==A10_BREAKOUT) max_fraction=g_breakout.Spread();
   else if(mode==A10_REENTRY) max_fraction=g_reentry.Spread();
   else if(mode==A10_MIDLINE) max_fraction=g_midline.Spread();
   else if(mode==A10_SQUEEZE) max_fraction=g_squeeze.Spread();
   else return false;
   ...
   for(int owner=0;owner<4;owner++)
     {
      ...
      const int direction=g_v_dir[owner];
      const double executable=(direction>0 ? tick.bid : tick.ask);
      const double brick=ModeBrick(owner);
      const double move=direction*(executable-g_v_price[owner]);
      if(ModeTP(owner)*brick>0.0 && move>=ModeTP(owner)*brick) { RequestExit("TP",owner,g_v_ticket[owner]); continue; }
      if(ModeSL(owner)*brick>0.0 && move<=-ModeSL(owner)*brick) { RequestExit("SL",owner,g_v_ticket[owner]); continue; }
      const int max_hold=ModeMaxHold(owner);
      if(max_hold>0 && g_v_time[owner]>0 && (long)(TimeCurrent()-g_v_time[owner])>=(long)max_hold*60)
         RequestExit("TIME",owner,g_v_ticket[owner]);
     }
  }

//+------------------------------------------------------------------+
void ProcessOwnerSignalExit(const int owner,const SA10TickResult &result)
  {
...
void UpdateAllModesAndTrade(const double price,const MqlTick &tick)
  {
   ...
   ProcessOwnerSignalExit(A10_BREAKOUT,r0);
   ProcessOwnerSignalExit(A10_REENTRY,r1);
   ProcessOwnerSignalExit(A10_MIDLINE,r2);
   ProcessOwnerSignalExit(A10_SQUEEZE,r3);
...
int OnInit(void)
  {
   ...
   if(!g_breakout.Configure(InpBreakoutEnabled,A10_BREAKOUT,tick_size,
      ...
      !g_reentry.Configure(InpReentryEnabled,A10_REENTRY,tick_size,
      ...
      !g_midline.Configure(InpMidlineEnabled,A10_MIDLINE,tick_size,
      ...
      !g_squeeze.Configure(InpSqueezeEnabled,A10_SQUEEZE,tick_size,
...
void OnDeinit(const int reason)
  {
   Print("[A10_NOORDERS_SUMMARY] ticks=",g_v_ticks," entries=",g_v_entries," exits=",g_v_exits,
