...
//+------------------------------------------------------------------+
// Observation-only abnormal lifecycle evidence counters.
long g_actual_partial_retcode=0;
long g_terminal_exit_position_still_present=0;
long g_terminal_exit_position_absent=0;
double g_last_requested_volume=0.0;
bool g_last_request_is_exit=false;
ulong g_last_request_order=0;

void A15AbnormalSnapshot(const string stage,const ulong order,const bool is_exit)
  {
   ulong ticket=0; long type=-1; double volume=0.0,price=0.0; datetime opened=0;
   const bool exists=FindOurPosition(ticket,type,volume,price,opened);
   PrintFormat("[A15_ABNORMAL_EVIDENCE] stage=%s order=%I64u is_exit=%d pos_found=%d pos_ticket=%I64u pos_volume=%.8f requested_volume=%.8f",
      stage,order,(int)is_exit,(int)exists,ticket,volume,g_last_requested_volume);
  }
   ...
   if(trans.type==TRADE_TRANSACTION_DEAL_ADD)
   ...
   else if(trans.type==TRADE_TRANSACTION_REQUEST && request.magic==GDS_MAGIC)
     {
      ...
      if(result.retcode==TRADE_RETCODE_DONE_PARTIAL)
        {
         g_actual_partial_retcode++;
         PrintFormat("[A15_ACTUAL_PARTIAL_FILL] order=%I64u deal=%I64u requested_volume=%.8f result_volume=%.8f retcode=%u ACTUAL_BROKER_EVENT=1",
            result.order,result.deal,request.volume,result.volume,result.retcode);
         A15AbnormalSnapshot("DONE_PARTIAL_REQUEST",result.order,g_last_request_is_exit);
        }
     }
  }
   ...
   if(g_wait_order!=0)
     {
      ...
      if(state!=ORDER_STATE_FILLED && state!=ORDER_STATE_CANCELED &&
         ...
         return false;
      if(g_last_request_is_exit && g_wait_order==g_last_request_order)
        {
         ulong ev_ticket=0; long ev_type=-1; double ev_volume=0.0,ev_price=0.0; datetime ev_opened=0;
         const bool ev_exists=FindOurPosition(ev_ticket,ev_type,ev_volume,ev_price,ev_opened);
         if(ev_exists)
           {
            g_terminal_exit_position_still_present++;
            PrintFormat("[A15_DELAYED_POSITION_UPDATE_CANDIDATE] order=%I64u terminal_state=%d pos_ticket=%I64u pos_volume=%.8f ACTUAL_BROKER_EVENT_CANDIDATE=1",
               g_wait_order,(int)state,ev_ticket,ev_volume);
           }
         else
           {
            g_terminal_exit_position_absent++;
            PrintFormat("[A15_TERMINAL_EXIT_POSITION_ABSENT] order=%I64u terminal_state=%d NORMAL_PATH=1",g_wait_order,(int)state);
           }
        }
      g_wait_order=0;
...
bool SubmitDeal(MqlTradeRequest &request,const bool is_exit)
  {
   ...
   g_last_requested_volume=request.volume;
   g_last_request_is_exit=is_exit;
   ...
   g_retry_after=0;
   g_wait_order=result.order;
   g_last_request_order=result.order;
...
void OnDeinit(const int reason)
  {
   PrintFormat("[A15_ABNORMAL_EVIDENCE_SUMMARY] partial_retcode=%I64d terminal_exit_position_present=%I64d terminal_exit_position_absent=%I64d PARTIAL_ACTUAL_CONFIRMED=%d DELAYED_UPDATE_ACTUAL_CANDIDATE=%d",
      g_actual_partial_retcode,g_terminal_exit_position_still_present,g_terminal_exit_position_absent,
      (int)(g_actual_partial_retcode>0),(int)(g_terminal_exit_position_still_present>0));
  }
