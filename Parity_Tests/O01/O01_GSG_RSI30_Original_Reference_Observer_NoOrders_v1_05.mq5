//+------------------------------------------------------------------+
//| O01_GSG_RSI30_Original_Reference_Observer_NoOrders_v1_05.mq5              |
//| Independent frozen-source reference observer. NO ORDERS.                        |
//+------------------------------------------------------------------+
#property strict
...
input bool InpUseNewsGateAdapter=false; input bool InpNewsBlocked=false; // tester adapter; false matches periods without captured calendar block
...
struct RefTrailState{bool active;double peak_pts,stop_pts;int position_count;}; RefTrailState bt,st;
...
void LO(string side,string tag,double px,double lot,int n){Print("[O01_REF105_OPEN] t=",TimeToString(TimeCurrent(),TIME_DATE|TIME_SECONDS)," side=",side," tag=",tag," count=",n," lot=",DoubleToString(lot,2)," px=",DoubleToString(px,_Digits)," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");}
void LX(string side,string reason,int n,double avg,double px,double mv){Print("[O01_REF105_EXIT] t=",TimeToString(TimeCurrent(),TIME_DATE|TIME_SECONDS)," side=",side," reason=",reason," count=",n," avg=",DoubleToString(avg,_Digits)," px=",DoubleToString(px,_Digits)," move_pts=",DoubleToString(mv,1)," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");}

string RefExit(bool isBuy,int n,double mv,RefTrailState &ts){
 int sl=InpVirtualSLPoints,start=n==1?InpSingleTrailStart:InpBasketTrailStart,lock=n==1?InpSingleTrailLock:InpBasketTrailLock,dist=n==1?InpSingleTrailDistance:InpBasketTrailDistance,step=n==1?InpSingleTrailStep:InpBasketTrailStep;
 if(sl>0&&mv<=-(double)sl)return "VIRTUAL_SL";
 if(ts.active&&ts.position_count!=n){ts.active=false;ts.peak_pts=0;ts.stop_pts=0;}
 if(!ts.active){if(start<=0||mv<(double)start){ts.position_count=n;return "NONE";}ts.active=true;ts.peak_pts=mv;ts.stop_pts=MathMax((double)lock,ts.peak_pts-(double)dist);ts.position_count=n;}
 else if(mv>ts.peak_pts&&(step<=0||mv-ts.peak_pts>=(double)step)){ts.peak_pts=mv;ts.stop_pts=MathMax(ts.stop_pts,MathMax((double)lock,ts.peak_pts-(double)dist));}
 if(ts.active&&mv<=ts.stop_pts)return n==1?"SINGLE_TRAILING":"BASKET_TRAILING";
 return "NONE";
}
void Manage(bool isBuy,VPos &p[],RefTrailState &ts,MqlTick &t){
 ...
 string d=RefExit(isBuy,n,mv,ts);if(d!="NONE"){LX(isBuy?"BUY":"SELL",d,n,avg,px,mv);closes++;if(d=="SINGLE_TRAILING")singleTrail++;if(d=="BASKET_TRAILING")basketTrail++;if(d=="VIRTUAL_SL")vsl++;Clear(p);ts.active=false;ts.peak_pts=0;ts.stop_pts=0;ts.position_count=0;return;}
...
int OnInit(){rh=iRSI(_Symbol,_Period,InpRSIPeriod,PRICE_CLOSE);a1h=iATR(_Symbol,_Period,InpATR1Period);a2h=iATR(_Symbol,InpATR2Timeframe,InpATR2Period);if(rh==INVALID_HANDLE||a1h==INVALID_HANDLE||a2h==INVALID_HANDLE)return INIT_FAILED;int s,e;Window(s,e);Print("[O01_REF105_START] AUTO_GMT_SOURCE_GATE=1 effective_server=",s/60,":",s%60,"-",e/60,":",e%60," offset=",DoubleToString(Offset(),2)," NEWS_GATE_ADAPTER=",(InpUseNewsGateAdapter?"ON":"OFF")," MAX_SPREAD=",InpMaxSpreadPoints," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");return INIT_SUCCEEDED;}
void OnDeinit(const int r){IndicatorRelease(rh);IndicatorRelease(a1h);IndicatorRelease(a2h);Print("[O01_REF105_SUMMARY] ticks=",ticks," entries=",entries," grids=",grids," closes=",closes," single_trail=",singleTrail," basket_trail=",basketTrail," virtual_sl=",vsl," buy_open=",C(buy)," sell_open=",C(sell)," time_blocks=",timeBlocks," news_blocks=",newsBlocks," spread_blocks=",spreadBlocks," filter_blocks=",filterBlocks," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");}
void OnTick(){
 ...
 int s=0;if(C(buy)==0&&r<InpRSILower)s=1;else if(C(sell)==0&&r>InpRSIUpper)s=-1;datetime bar=iTime(_Symbol,_Period,0);
 if(s==1&&(!InpOneOrderPerBar||lastBuyBar!=bar)){double l=NormLot(InpInitialLot);Add(buy,t.ask,l);lastBuyBar=bar;entries++;LO("BUY","INITIAL",t.ask,l,C(buy));}
 if(s==-1&&(!InpOneOrderPerBar||lastSellBar!=bar)){double l=NormLot(InpInitialLot);Add(sell,t.bid,l);lastSellBar=bar;entries++;LO("SELL","INITIAL",t.bid,l,C(sell));}
}