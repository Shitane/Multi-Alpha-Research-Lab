...
int OnInit(){Defaults();if(!adapter.Validate(runtime_cfg))return INIT_PARAMETERS_INCORRECT;rh=iRSI(_Symbol,_Period,runtime_cfg.rsi_period,PRICE_CLOSE);a1h=iATR(_Symbol,_Period,runtime_cfg.atr1_period);a2h=iATR(_Symbol,runtime_cfg.atr2_timeframe,runtime_cfg.atr2_period);if(rh==INVALID_HANDLE||a1h==INVALID_HANDLE||a2h==INVALID_HANDLE)return INIT_FAILED;panel.Create(runtime_cfg);Print("[O01_RUNTIME121_START] CORE=1.00 ADAPTER=1.20 PANEL=1.20 NO_ORDERS=1 VIRTUAL_NOT_FILL=1");return INIT_SUCCEEDED;}
