//+------------------------------------------------------------------+
//| O01_GSG_RSI30_Runtime_Panel_Core_NoOrders_v1_30.mq5            |
...
#include "..\\..\\Include\\O01\\O01_Settings_Panel_v1_30.mqh"

enum O01_TIME_MODE { O01_AUTO_GMT=0,O01_SERVER_TIME=1,O01_CUSTOM_GMT=2 };
SO01RuntimeSettings110 runtime_cfg; CO01SettingsPanel130 panel; CO01RuntimeAdapter120 adapter; CO01CoreInterface core;
...
void LO(string side,string tag,double px,double lot,int n){Print("[O01_RUNTIME130_OPEN] t=",TimeToString(TimeCurrent(),TIME_DATE|TIME_SECONDS)," side=",side," tag=",tag," count=",n," lot=",DoubleToString(lot,2)," px=",DoubleToString(px,_Digits)," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");}
void LX(string side,ENUM_O01_EXIT_DECISION d,int n,double avg,double px,double mv){Print("[O01_RUNTIME130_EXIT] t=",TimeToString(TimeCurrent(),TIME_DATE|TIME_SECONDS)," side=",side," reason=",EN(d)," count=",n," avg=",DoubleToString(avg,_Digits)," px=",DoubleToString(px,_Digits)," move_pts=",DoubleToString(mv,1)," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");}
...
int OnInit(){Defaults();if(!adapter.Validate(runtime_cfg))return INIT_PARAMETERS_INCORRECT;rh=iRSI(_Symbol,_Period,runtime_cfg.rsi_period,PRICE_CLOSE);a1h=iATR(_Symbol,_Period,runtime_cfg.atr1_period);a2h=iATR(_Symbol,(ENUM_TIMEFRAMES)runtime_cfg.atr2_timeframe,runtime_cfg.atr2_period);if(rh==INVALID_HANDLE||a1h==INVALID_HANDLE||a2h==INVALID_HANDLE)return INIT_FAILED;panel.Create(runtime_cfg);Print("[O01_RUNTIME130_START] CORE=1.00 ADAPTER=1.20 PANEL=1.30 NO_ORDERS=1 VIRTUAL_NOT_FILL=1");return INIT_SUCCEEDED;}
void OnDeinit(const int reason){panel.Delete();if(rh!=INVALID_HANDLE)IndicatorRelease(rh);if(a1h!=INVALID_HANDLE)IndicatorRelease(a1h);if(a2h!=INVALID_HANDLE)IndicatorRelease(a2h);Print("[O01_RUNTIME130_SUMMARY] ticks=",ticks," entries=",entries," grids=",grids," closes=",closes," single_trail=",singleTrail," basket_trail=",basketTrail," virtual_sl=",vsl," buy_open=",C(buy)," sell_open=",C(sell)," time_blocks=",timeBlocks," news_blocks=",newsBlocks," spread_blocks=",spreadBlocks," filter_blocks=",filterBlocks," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");}
void OnTick(){
...
void OnChartEvent(const int id,const long &lparam,const double &dparam,const string &sparam){int r=panel.Event(id,sparam,runtime_cfg);if(r!=0){bool ok=adapter.Validate(runtime_cfg);Print("[O01_RUNTIME130_PANEL] result=",r," valid=",(int)ok," NO_ORDERS=1");}}
