//+------------------------------------------------------------------+
//| O01_GSG_RSI30_Virtual_Lifecycle_NoOrders_v1_04.mq5              |
//| Exact entry-gate adapter step. NO ORDERS.                        |
//+------------------------------------------------------------------+
#property strict
...
SO01TrailState bt,st; CO01EntryModule entry; CO01ExitModule exits;
...
void LO(string side,string tag,double px,double lot,int n){Print("[O01_V104_OPEN] t=",TimeToString(TimeCurrent(),TIME_DATE|TIME_SECONDS)," side=",side," tag=",tag," count=",n," lot=",DoubleToString(lot,2)," px=",DoubleToString(px,_Digits)," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");}
void LX(string side,ENUM_O01_EXIT_DECISION d,int n,double avg,double px,double mv){Print("[O01_V104_EXIT] t=",TimeToString(TimeCurrent(),TIME_DATE|TIME_SECONDS)," side=",side," reason=",EN(d)," count=",n," avg=",DoubleToString(avg,_Digits)," px=",DoubleToString(px,_Digits)," move_pts=",DoubleToString(mv,1)," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");}
...
void Manage(bool isBuy,VPos &p[],SO01TrailState &ts,MqlTick &t){
 ...
 ENUM_O01_EXIT_DECISION d=exits.Evaluate(isBuy,n,mv,c,ts);if(d!=O01_EXIT_NONE){LX(isBuy?"BUY":"SELL",d,n,avg,px,mv);closes++;if(d==O01_EXIT_SINGLE_TRAILING)singleTrail++;if(d==O01_EXIT_BASKET_TRAILING)basketTrail++;if(d==O01_EXIT_VIRTUAL_SL)vsl++;Clear(p);ts.active=false;ts.peak_pts=0;ts.stop_pts=0;ts.position_count=0;return;}
...
int OnInit(){rh=iRSI(_Symbol,_Period,InpRSIPeriod,PRICE_CLOSE);a1h=iATR(_Symbol,_Period,InpATR1Period);a2h=iATR(_Symbol,InpATR2Timeframe,InpATR2Period);if(rh==INVALID_HANDLE||a1h==INVALID_HANDLE||a2h==INVALID_HANDLE)return INIT_FAILED;int s,e;Window(s,e);Print("[O01_V104_START] AUTO_GMT_SOURCE_GATE=1 effective_server=",s/60,":",s%60,"-",e/60,":",e%60," offset=",DoubleToString(Offset(),2)," NEWS_GATE_ADAPTER=",(InpUseNewsGateAdapter?"ON":"OFF")," MAX_SPREAD=",InpMaxSpreadPoints," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");return INIT_SUCCEEDED;}
void OnDeinit(const int r){IndicatorRelease(rh);IndicatorRelease(a1h);IndicatorRelease(a2h);Print("[O01_V104_SUMMARY] ticks=",ticks," entries=",entries," grids=",grids," closes=",closes," single_trail=",singleTrail," basket_trail=",basketTrail," virtual_sl=",vsl," buy_open=",C(buy)," sell_open=",C(sell)," time_blocks=",timeBlocks," news_blocks=",newsBlocks," spread_blocks=",spreadBlocks," filter_blocks=",filterBlocks," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");}
void OnTick(){
 ...
 SO01EntryConfig c;c.new_cycles=true;c.trade_buy=true;c.trade_sell=true;c.rsi_lower=InpRSILower;c.rsi_upper=InpRSIUpper;SO01EntryContext x;x.emergency_lock=false;x.time_allowed=true;x.news_blocked=false;x.spread_ok=true;x.filters_ok=true;x.buy_count=C(buy);x.sell_count=C(sell);x.rsi=r;ENUM_O01_ENTRY_SIGNAL s=entry.Evaluate(c,x);datetime bar=iTime(_Symbol,_Period,0);
