//+------------------------------------------------------------------+
//| O01_GSG_RSI30_Virtual_Lifecycle_NoOrders_v1_05.mq5              |
//| Split Entry/Exit parity candidate against REF105. NO ORDERS.     |
//+------------------------------------------------------------------+
#property strict
...
void LO(string side,string tag,double px,double lot,int n){Print("[O01_SPLIT105_OPEN] t=",TimeToString(TimeCurrent(),TIME_DATE|TIME_SECONDS)," side=",side," tag=",tag," count=",n," lot=",DoubleToString(lot,2)," px=",DoubleToString(px,_Digits)," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");}
void LX(string side,ENUM_O01_EXIT_DECISION d,int n,double avg,double px,double mv){Print("[O01_SPLIT105_EXIT] t=",TimeToString(TimeCurrent(),TIME_DATE|TIME_SECONDS)," side=",side," reason=",EN(d)," count=",n," avg=",DoubleToString(avg,_Digits)," px=",DoubleToString(px,_Digits)," move_pts=",DoubleToString(mv,1)," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");}
...
int OnInit(){rh=iRSI(_Symbol,_Period,InpRSIPeriod,PRICE_CLOSE);a1h=iATR(_Symbol,_Period,InpATR1Period);a2h=iATR(_Symbol,InpATR2Timeframe,InpATR2Period);if(rh==INVALID_HANDLE||a1h==INVALID_HANDLE||a2h==INVALID_HANDLE)return INIT_FAILED;int s,e;Window(s,e);Print("[O01_SPLIT105_START] REF=O01_REF105 effective_server=",s/60,":",s%60,"-",e/60,":",e%60," offset=",DoubleToString(Offset(),2)," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");return INIT_SUCCEEDED;}
void OnDeinit(const int r){IndicatorRelease(rh);IndicatorRelease(a1h);IndicatorRelease(a2h);Print("[O01_SPLIT105_SUMMARY] ticks=",ticks," entries=",entries," grids=",grids," closes=",closes," single_trail=",singleTrail," basket_trail=",basketTrail," virtual_sl=",vsl," buy_open=",C(buy)," sell_open=",C(sell)," time_blocks=",timeBlocks," news_blocks=",newsBlocks," spread_blocks=",spreadBlocks," filter_blocks=",filterBlocks," NO_ORDERS=1 VIRTUAL_NOT_FILL=1");}
void OnTick(){
